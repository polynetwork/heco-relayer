...
type proofRsp struct {
	JsonRPC string     `json:"jsonrpc"`
	Result  HecoProof  `json:"result,omitempty"`
	Error   *jsonError `json:"error,omitempty"`
	Id      uint       `json:"id"`
}

type HecoProof struct {
...
func GetExplorerUrl(chainId uint64) string {
	...
	case uint64(128): // refer to https://docs.hecochain.com/#/en-us/mainnet?id=chainid
		return "https://scan.hecochain.com/tx/"
	case uint64(256): // refer to https://docs.hecochain.com/#/en-us/testnet?id=chainid
		return "https://scan-testnet.hecochain.com/tx/"
	default:
		return "no url"
	}
}
