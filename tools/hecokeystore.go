...
type HecoKeyStore struct {
	ks      *keystore.KeyStore
	chainId *big.Int
}

func NewHecoKeyStore(sigConfig *config.HecoConfig, chainId *big.Int) *HecoKeyStore {
	service := &HecoKeyStore{}
...
func (this *HecoKeyStore) UnlockKeys(sigConfig *config.HecoConfig) error {
	for _, v := range this.GetAccounts() {
		err := this.ks.Unlock(v, sigConfig.KeyStorePwdSet[strings.ToLower(v.Address.String())])
		if err != nil {
			return fmt.Errorf("failed to unlock heco acc %s: %v", v.Address.String(), err)
		}
	}
	return nil
}

func (this *HecoKeyStore) SignTransaction(tx *types.Transaction, acc accounts.Account) (*types.Transaction, error) {
...
func (this *HecoKeyStore) GetAccounts() []accounts.Account {
	return this.ks.Accounts()
}

func (this *HecoKeyStore) TestPwd(acc accounts.Account, pwd string) error {
...
func (this *HecoKeyStore) GetChainId() uint64 {
